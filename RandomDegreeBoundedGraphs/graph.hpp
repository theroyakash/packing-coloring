...
Graph generateGrowthBoundedGraphsWithDegreeBound(int n, int d) {
    Graph G(n);
    ...
    for (int i = 2; i <= n; i++) {
        int random = s.getRandom();
        while (degree[i] >= d and degree[random] >= d) {
            random = s.getRandom();
        }

        G.add_edge(i, random);
        degree[i]++; degree[random]++;
...
int countTotalLeaves(Graph g) {
    int totalNodes = g.maxNodes;
    int totalLeaves = 0;

    for (int i = 1; i <= totalNodes; i++) {
        if (g.adj_list[i].size() == 1) {
            totalLeaves++;
        }
    }
